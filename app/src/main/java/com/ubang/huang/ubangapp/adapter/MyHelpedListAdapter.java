 ...
 * 我发出求助的列表适配器
 */
public class MyHelpedListAdapter extends RecyclerView.Adapter<MyHelpedListAdapter.ViewHolder> {
    ...
    public MyHelpedListAdapter(List<SeekHelpInfo> helpInfoList, Context context) {
    ...
    private void addRecycle(ViewHolder viewHolder, SeekHelpInfo help) {
//        BaseRecycleView baseRecycleView = new BaseRecycleView(context);
//        viewHolder.recyclerView = baseRecycleView.addRecycle(viewHolder.recyclerView, CP.Grid,false,true);
//        addHandle(viewHolder);
        ...
        if(help.getHas_picture() == 1) {
            picAdapter = new NetPicAdapter(help.getPisList(), context);
                ...
                switch(msg.what) {
                    case Signal.GetHelpPic:
                        UIHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                viewHolder.recyclerView.loadMoreComplete();
